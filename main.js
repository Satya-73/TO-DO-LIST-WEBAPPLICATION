...
function onEditTodo(todoId, labelId) {
    ...
    let newText = prompt("Edit task:", labelElement.textContent);

    if (newText !== null) {
        labelElement.textContent = newText;

        // Update the corresponding todo in the todoList array
        let editElementIndex = todoList.findIndex(function(eachTodo) {
            let eachTodoId = "todo" + eachTodo.uniqueNo;
            return eachTodoId === todoId;
        });

        if (editElementIndex !== -1) {
            todoList[editElementIndex].text = newText;
        }
    }
}
